...
from fasthtml.common import Script, Div, Title, Link, ft_hx, FastHTML, Button, Svg
from fasthtml.svg import SvgInb, Circle
from fasthtml.components import Defs, ClipPath, Rect, Image, G
import signal
import uvicorn
from fastapi.responses import FileResponse
from components import menu, split, sidebar, tile_bar, canvas
import sys
import os
import time
...
class Parameters:
    ...
    doubles_removal_distance: int = 100
    detected_circles = []
    ...
    return (
        ...
        Div(
            ...
            Div(
                ...
                Div(
                    # Main Canvas
                    canvas.main_canvas(current_settings),
                    tile_bar.tile_bar(current_settings),
                    cls="flex flex-col",
                    id="split-0",
                    x_data="{ regionSize: 400 }"
                ),
                Div(
                    sidebar.sidebar(current_settings, update_preview),
                    id="split-1",
...
def add_scan():
    ...
    return sidebar.sidebar(current_settings, update_preview)
...
def process_image():
    ...
    cancel_event.clear()  # Reset the cancellation event at the start of processing
    ...
    try:
        for i in current_settings.scan_parameters:
            ...
            if cancel_event.is_set():   # If processing was canceled, return early with full cleanup
...
def cancel_success():
    ...
    while processing_active:    # Wait until processing_active is False
